...
export function createEditImports(name: string, fields: Field[], arrays: string[]): string {
  const imports: string[] = [
    ...
    `import { BaseCreateEditComponent${ arrays.length > 0 ? ', DialogContentWithOptionsInterface' : '' } } from 'ddata-core';`,
  ...
  fields.filter(item => item.is_array === true).forEach((field: Field) => {
    imports.push(
      ...
      `import { ${ field.model_name }CreateEditComponent } from 'src/app/components/${ slasherize(field.model_name) }/${ dasherize(field.model_name) }-create-edit.component';`,
      `import { ${ field.model_name }ListComponent } from 'src/app/components/${ slasherize(field.model_name) }/${ dasherize(field.model_name) }-list.component';`,
      );
  })
